...
//
// CHANGE (per update doc): sector hubs now get FIXED anchors (fx/fy) at 4 quadrant
// positions so d3-force's charge/link forces can't drag them toward center over
// time -- keeps the aggregate view's 4-quadrant layout stable and "breathable".
// Lenders are placed on a surrounding ring (unanchored, so they still settle
// naturally via the simulation) rather than spawning stacked at the origin.
export function buildAggregateGraph(data) {
  const { sectors, lenders, loans } = data;

  // Pre-position the 4 sector super-hubs at 4 quadrants so d3-force doesn't
  // collapse them to center. fx/fy (not just x/y) makes this a hard pin.
  const positions = [
    { x: -320, y: -200 }, { x: 320, y: -200 },
    { x: -320, y:  200 }, { x: 320, y:  200 },
  ];
  ...
  const nodes = [
    ...sectors.map((s, i) => ({
      ...
      fx: positions[i % positions.length].x, // fixed anchor -- hub stays put
      fy: positions[i % positions.length].y,
    })),
    ...lenders.map((l, i) => ({
      id: l.id,
      label: l.name,
      type: 'lender',
      val: 6,
      x: 480 * Math.cos((i / lenders.length) * 2 * Math.PI),
      y: 480 * Math.sin((i / lenders.length) * 2 * Math.PI),
      // no fx/fy here -- lenders stay free to settle via simulation
    })),
  ];
